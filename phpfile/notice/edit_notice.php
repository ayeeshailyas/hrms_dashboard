...
<style>
    ...
    /* Force dropdown to not overflow */
    .cke_combopanel {
...
<?php if (isset($_GET['msg'])): ?>
    <div class="custom-alert <?= $_GET['msg'] ? 'show' : '' ?>" id="alertBox">
        <?php
        if ($_GET['msg'] == 'success') echo 'Notice saved successfully.';
        elseif ($_GET['msg'] == 'deleted') echo 'Notice deleted successfully.';
        elseif ($_GET['msg'] == 'updated') echo 'Notice updated successfully.';
        ?>
    </div>
<?php endif; ?>
...
<div id="mainContent" class=" p-8 absolute top-1 ">
    <h1 class="font-medium text-[22px]">Add Notice</h1>
    <span>
    ...
    <div class="max-w-5xl mx-auto bg-white shadow p-6 rounded-md border-t-4 border-teal-500">
    <h3 class="text-lg font-semibold mb-4">Edit Notice</h3>

    <form action="update_notice.php" method="POST" class="space-y-5">

        <input type="hidden" name="id" value="<?= $row['id'] ?>">

        <!-- Publication Status -->
        <div class="lg:flex md:flex items-center lg:space-x-4 md:space-x-4 mb-4">
            <label class="w-40 text-[15px] text-gray-700 sm:pb-2 lg:pb-0 md:pb-0 pb-2">
            ...
            <label class="flex items-center space-x-2 cursor-pointer group">
                <div class="relative">
                    <input type="checkbox" name="status" value="Published"
                           class="absolute opacity-0 h-0 w-0 peer status-checkbox"
                        <?= $row['status'] == 'Published' ? 'checked' : '' ?>>
                    <div class="checkbox-container border-2 border-[#01a9ac] flex items-center justify-center">
                        <svg class="checkmark" viewBox="0 0 24 24" fill="none" stroke="#9ca3af"
                             stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
                            <path class="checkmark-path" d="M20 6L9 17l-5-5"
                                  style="stroke-dasharray: 30; stroke-dashoffset: <?= $row['status'] == 'Published' ? '0' : '30' ?>;" />
                        </svg>
                    </div>
                </div>
            ...
            <label class="flex items-center space-x-2 cursor-pointer group">
                <div class="relative">
                    <input type="checkbox" name="status" value="Unpublished"
                           class="absolute opacity-0 h-0 w-0 peer status-checkbox"
                        <?= $row['status'] == 'Unpublished' ? 'checked' : '' ?>>
                    <div class="checkbox-container border-2 border-[#01a9ac] flex items-center justify-center">
                        <svg class="checkmark" viewBox="0 0 24 24" fill="none" stroke="#9ca3af"
                             stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
                            <path class="checkmark-path" d="M20 6L9 17l-5-5"
                                  style="stroke-dasharray: 30; stroke-dashoffset: <?= $row['status'] == 'Unpublished' ? '0' : '30' ?>;" />
                        </svg>
                    </div>
                </div>
        ...
        <div class="lg:flex md:flex items-center lg:space-x-4 md:space-x-4 ">
            <label class="w-48 text-[15px]  text-gray-700 sm:pb-2 lg:pb-0 md:pb-0 pb-2">Title <span class="text-red-500">*</span></label>
            <input type="text" name="title" placeholder="Enter Notice Title Here" value="<?= htmlspecialchars($row['title']) ?>" required
                   class="form-control w-full rounded-none border border-gray-300 focus:border-[#01a9ac]" />
        </div>

        <!-- short discription -->
        <div class="lg:flex md:flex items-start lg:space-x-4 md:space-x-4 ">
            <label class="w-48 text-[15px]  text-gray-700 sm:pb-2 lg:pb-0 md:pb-0 pb-2">Short Description <span class="text-red-500">*</span></label>
            <textarea  name="short_desc" placeholder="Enter Short Description"  required
                       class="form-control w-full rounded-none border border-gray-300 focus:border-[#01a9ac]" ><?= htmlspecialchars($row['short_desc']) ?></textarea>
        </div>


        <!-- long description -->
        <div class="lg:flex md:flex items-start lg:space-x-4 md:space-x-4 ">
            <label class="w-48 text-[15px] text-gray-700 pt-2 ">Long Description <span class="text-red-500">*</span></label>
            <div class="w-full">
        <textarea name="long_desc" id="long_desc" required
                  class="form-control rounded-none border border-gray-300 focus:border-[#01a9ac]"><?= $row['long_desc'] ?></textarea>
            </div>
        </div>


        <!-- Submit Button -->
        <div class="text-start pt-2 lg:pl-[10.8rem] md:pl-[11rem] sm:pl-0 pl-0">
            <button type="submit" class="bg-[#01a9ac] hover:bg-[#07e4e8] text-white text-sm px-3 py-2 transition-all duration-300">Update</button>
        </div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        checkboxes.forEach(cb => {
            cb.addEventListener('change', function () {
                ...
                // Visual stroke update
                checkboxes.forEach(box => {
                    const svg = box.parentElement.querySelector('svg');
                    const path = svg.querySelector('.checkmark-path');
                    if (box.checked) {
                        svg.setAttribute('stroke', '#01a9ac');
