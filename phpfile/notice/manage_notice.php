...
<div id="mainContent" class=" p-8 absolute top-1 ">
    <h1 class="font-medium text-[22px]">Notice</h1>
    <span>
    ...
    <div class="w-full mx-auto bg-white p-6 shadow rounded-md border-t-4 border-teal-500">
        <h3 class="text-md font-semibold mb-4">List of All Notices</h3>
        ...
        <table class="table table-bordered table-hover align-middle text-sm">
            <thead>
            <tr>
                <th class="font-medium text-[15px] py-3">Sr.</th>
                <th class="font-medium text-[15px] py-3">Created Date</th>
                <th class="font-medium text-[15px] py-3">Title</th>
                <th class="font-medium text-[15px] py-3">Short Description</th>
                <th class="font-medium text-[15px] py-3">Status</th>
                <th class="font-medium text-center text-[15px] py-3">Action</th>
            </tr>
            </thead>
            <tbody>
            <?php
            ...
            while ($row = $result->fetch_assoc()):
                ...
                <tr>
                    ...
                    <td class="py-3"><?= date("d M Y, h:i A", strtotime($row['created_at'])) ?></td>
                    <td class="py-3"><?= htmlspecialchars($row['title']) ?></td>
                    <td class="py-3"><?= htmlspecialchars($row['short_desc']) ?></td>
                    <td class="py-3 text-center">

                        <span class="badge" style="background: <?= $row['status'] === 'Published' ? 'linear-gradient(to right, #0ac282, #0df3a3)' : '#6c757d' ?>; color: white; padding: 8px 12px;">
                         <?= $row['status'] ?>
                        </span>

                    </td>
                    <td class="text-center py-3">
                        <a href="view_notice.php?id=<?= $row['id'] ?>" class="btn bg-[#2DCEE3] px-[14px] py-1.5 rounded-1 hover:bg-[#2fdef6] btn-sm text-white"><i class="fa fa-list-alt"></i></a>
                        <a href="edit_notice.php?id=<?= $row['id'] ?>" class="bg-[#01a9ac] hover:bg-[#07e4e8] rounded-1 text-white text-xs px-4 py-2.5 transition-all duration-300"><i class="fa fa-edit"></i> Edit</a>
                        <a href="delete_notice.php?id=<?= $row['id'] ?>" onclick="return confirm('Are you sure you want to delete this notice?')" class="bg-[#fe5d70] rounded-1 text-white px-3 py-2.5 text-xs hover:bg-[#fe5d70] transition-all duration-300"><i class="fa fa-trash-o"></i> Delete</a>
...
<div style="background-color: #2fdef6"></div>
