...
// Fetch employee data
$emp_query = $conn->prepare("SELECT * FROM employee WHERE id = ?");
$emp_query->bind_param("i", $emp_id);
$emp_query->execute();
$employee = $emp_query->get_result()->fetch_assoc();

// Fetch salary details
$salary_query = $conn->prepare("SELECT * FROM salary_details WHERE emp_id = ? ORDER BY created_at DESC LIMIT 1");
$salary_query->bind_param("i", $emp_id);
$salary_query->execute();
$salary = $salary_query->get_result()->fetch_assoc();
...
");
$payment_query->bind_param("iii", $emp_id, $year, $month);
$payment_query->execute();
$payment = $payment_query->get_result()->fetch_assoc();

// Get payslip number (count of all previous payments for this employee up to current month)
$payslip_no_query = $conn->prepare("
    SELECT COUNT(*) as count FROM salary_payments 
    WHERE emp_id = ? 
    AND (YEAR(payment_date) < ? OR (YEAR(payment_date) = ? AND MONTH(payment_date) <= ?))
");
$payslip_no_query->bind_param("iiii", $emp_id, $year, $year, $month);
$payslip_no_query->execute();
$payslip_no_result = $payslip_no_query->get_result()->fetch_assoc();
$payslip_no = $payslip_no_result ? $payslip_no_result['count'] : 0;
$payslip_no++; // Start from 01

// Format date and numbers
$formatted_date = date('d-M, Y', strtotime($payment['payment_date'] ?? date('Y-m-d')));
$month_name = date('F Y', strtotime($salary_month . '-01'));
...
<div id="mainContent" class=" pt-8 pl-8 pr-8 absolute top-1 ">
    <h1 class="font-semibold text-[24px]">Generate Payslip</h1>
    <span>
    ...
    <div class="w-full mx-auto p-6">
        ...
        <div class="flex float-left">
            <img src="../../../images/img_2.png" class="w-12 h-12 rounded-circle" alt="">
            <p class="font-medium pt-2 pl-3">Almahir Hub</p>
        </div>

        <div class=" float-right">
            <button onclick="window.print()"
                    style="padding: 8px 18px; background-color: #01a9ac; color: white; border: none; border-radius: 3px; cursor: pointer;">
                Print Payslip
            </button>
        </div>
    </div>
    ...
    <div class="py-3"></div>
    <div class="border-b-2 border-teal-950"></div>
    <div class="py-4"></div>
    <div class="w-full mx-auto p-6 bg-[#f3f3f3] rounded">

        <div class="payslip-header">

            <h1 class="font-bold text-lg text-black">Payslip</h1>
            <h2 class="font-bold text-lg text-black">Salary Month: <?= $month_name ?></h2>
        </div>
        <div class="py-2"></div>
        <div class="grid grid-cols-3 gap-2">
            ...
            <div class="col-span-1">
                <div class="space-y-3">
                    <p><strong>Payslip No: </strong><?= str_pad($payslip_no, 2, '0', STR_PAD_LEFT) ?></p>
                    <p><strong>Address:</strong> <?= $employee['address'] . ', ' . $employee['city'] ?></p>
                    <p><strong>Joining Date:</strong> <?= date('d-M, Y', strtotime($employee['join_date'])) ?></p>
                </div>
            </div>
        </div>
        <div class="py-2"></div>
    ...
    <div class="w-full bg-[#d5dae6]  pl-6 pr-6">
        <div class="grid grid-cols-12 gap-4">
            <div class="lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                <div class="py-2"></div>
                <h1 class="font-bold text-[22px] tracking-normal">Earning</h1>
                <table class="table-bordered border-black">
                    ...
                    <tr>
                        <td class="text-end font-medium">Basic Salary:</td>
                        <td class="text-left"><?= number_format($salary['basic_salary'] ?? 0, 2) ?></td>
                    </tr>
                    <tr>
                        <td class="text-end font-medium">House Rent Allowance:</td>
                        <td class="text-left"><?= number_format($salary['house_allowance'] ?? 0, 2) ?></td>
                    </tr>
                    <tr>
                        <td class="text-end font-medium">Medical Allowance:</td>
                        <td class="text-left"><?= number_format($salary['medical_allowance'] ?? 0, 2) ?></td>
                    </tr>
                    <tr>
                        <td class="text-end font-medium">Special Allowance:</td>
                        <td class="text-left"><?= number_format($salary['special_allowance'] ?? 0, 2) ?></td>
                    </tr>
                    <tr>
                        <td class="text-end font-medium">Fuel Allowance:</td>
                        <td class="text-left"><?= number_format($salary['fuel_allowance'] ?? 0, 2) ?></td>
                    </tr>
                    <tr>
                        <td class="text-end font-medium">Phone Bill Allowance:</td>
                        <td class="text-left"><?= number_format($salary['phone_allowance'] ?? 0, 2) ?></td>
                    </tr>
                    <tr>
                        <td class="text-end font-medium">Other Allowance:</td>
                        <td class="text-left"><?= number_format($salary['other_allowance'] ?? 0, 2) ?></td>
                    </tr>
                </table>
            </div>
            ...
            <div class="lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                <div class="py-2"></div>
                <h3 class="font-bold text-[22px] tracking-normal">Deductions</h3>
                <table class="table-bordered border-black">
                    <tr>
                    ...
                    <tr>
                        <td class="text-end font-medium">Provident Fund:</td>
                        <td class="text-left"><?= number_format($salary['provident_fund'] ?? 0, 2) ?></td>
                    </tr>
                    <tr>
                        <td class="text-end font-medium">Tax Deduction:</td>
                        <td class="text-left"><?= number_format($salary['tax_deduction'] ?? 0, 2) ?></td>
                    </tr>
                    <tr>
                        <td class="text-end font-medium">Other Deduction:</td>
                        <td class="text-left"><?= number_format($salary['other_deduction'] ?? 0, 2) ?></td>
                    </tr>
                </table>

                <div class="py-4"></div>

                <h1 class="font-bold text-[22px] tracking-normal">Total Details</h1>
                <table>
                    <tr>
                        <td class="text-end font-medium">Gross Salary:</td>
                        <td class="text-left"><?= number_format($payment['gross_salary'] ?? $salary['gross_salary'] ?? 0, 2) ?></td>
                    </tr>
                    <tr>
                        <td class="text-end font-medium">Total Deduction:</td>
                        <td class="text-left"><?= number_format($payment['total_deduction'] ?? $salary['total_deduction'] ?? 0, 2) ?></td>
                    </tr>
                    <tr>
                        <td class="text-end font-medium">Net Salary:</td>
                        <td class="text-left"><?= number_format($payment['net_salary'] ?? $salary['net_salary'] ?? 0, 2) ?></td>
                    </tr>
                    <?php if ($payment_amount > 0): ?>
                        <tr class="bg-[#F6F7FB]">
                            <td class="text-end font-medium">Payment Amount:</td>
                            <td class="text-left"><?= number_format($payment_amount, 2) ?></td>
                        </tr>
                    <?php endif; ?>
                </table>
...
<script>
    $(document).ready(function () {
        ...
        // Update the goBtn click handler
        $('#goBtn').click(function () {
            let emp_id = $('#employeeDropdown').val();
            let salary_month = $('#dobInput').val();
            ...
            // Update heading
            $('#payslipHeading').text(`${monthName} ${year}`).removeClass('hidden');

            $.get('fetch_salary_data.php', {
                emp_id: emp_id,
                salary_month: salary_month
            }, function (data) {
                $('#salaryTableBody').html(data);
                $('#salarySection').show();
            });
        });
    });
...
<script>
    ...
    actionBtn.onclick = () => {
        if (gridView.classList.contains("hidden")) {
            ...
            // Only set value when user selects a date
            dobInput.value = `${tempDate.getFullYear()}-${String(tempDate.getMonth() + 1).padStart(2, "0")}`;
