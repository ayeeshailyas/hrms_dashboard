...
$sql = "SELECT e.id as emp_id, s.id as sid, e.f_name, e.l_name, 
        s.gross_salary, s.total_deduction, s.net_salary, s.enrollment_type 
...
<div id="mainContent" class=" p-8 absolute top-1 ">
    <h1 class="font-semibold text-[24px]">Employee Salary List</h1>
    <span>
<!--    <p class="text-[#919aa3] text-[14px] pt-2 float-left">Holiday List</p>-->
    ...
    <div class="w-full mx-auto bg-white shadow p-6 rounded-md border-t-4 border-teal-500">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-md font-medium text-gray-800">List of Employees and Their Salaries</h1>
            <div>
                <a href="#" onclick="generatePDF()"
                   class="bg-[#01a9ac] text-white text-[14px] px-3 py-2.5  hover:bg-[#07e4e8] transition-all"> <i
                            class="fa fa-file-pdf-o"></i></a>
                <a href="#" onclick="window.print()"
                   class="bg-[#01a9ac] text-white text-[14px] px-3 py-2.5  hover:bg-[#07e4e8] transition-all ml-2"> <i
        ...
        <table class="table-bordered border-[#ececec] w-full text-sm" cellpadding="10">
            ...
            <?php if ($result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>Rs. <?= number_format($row['gross_salary'], 2) ?></td>
                        <td>Rs. <?= number_format($row['total_deduction'], 2) ?></td>
                        <td>Rs. <?= number_format($row['net_salary'], 2) ?></td>
                        <td><?= htmlspecialchars($row['enrollment_type']) ?></td>
                        <td class="text-center">
                            <a href="salary_details_view.php?id=<?= $row['sid'] ?>" class="btn bg-[#2DCEE3] px-[14px] py-1.5 rounded-1 hover:bg-[#2fdef6] btn-sm text-white"><i class="fa fa-list-alt"></i></a>
                        </td>



                        <td class="text-center">
                            <a href="edit_salary.php?id=<?= $row['emp_id'] ?>" class="bg-[#01a9ac] hover:bg-[#07e4e8] text-white  transition-all ease-in duration-300 rounded-[2px] px-3 py-2">
            ...
            <?php else: ?>
                <tr>
                    <td colspan="8" class="text-center text-danger">No salary records found.</td>
                </tr>
            <?php endif; ?>
            </tbody>
        </table>
