...
<div id="mainContent" class=" p-8 absolute top-3 ">
    <h1 class="font-semibold text-[24px]">Leave Category List</h1>
    <span>
    ...
    <div id="alertBox" class="fixed top-5 right-[-400px] bg-black text-white px-6 py-3 rounded shadow transition-all duration-500 z-50">
      <span>
        <?= $_GET['msg'] === 'added' ? 'Category added successfully' : 'Category updated successfully' ?>
      </span>
    </div>
    <script>
        setTimeout(() => {
            document.getElementById("alertBox").style.right = "20px";
        }, 300);
        setTimeout(() => {
            document.getElementById("alertBox").style.right = "-400px";
        }, 3000);
    </script>
<?php endif; ?>
...
<div class="grid grid-cols-12 gap-6">
    <!-- Add/Edit Form -->
    <div class="lg:col-span-4 md:col-span-4 sm:col-span-12 col-span-12  bg-white rounded shadow p-3  border-t-4 border-teal-500 self-start">
        <h2 class="font-medium text-gray-700 text-[15px] pt-2 pb-4"><?= $editData ? 'Edit Category' : 'Add Category' ?></h2>
        <form method="POST">
            <input type="text" name="name" placeholder="Enter Category Name"
                   class="w-full border border-gray-700 px-3 py-2 text-sm mb-3"
                   value="<?= $editData['name'] ?? '' ?>" required>

            <?php if ($editData): ?>
                <input type="hidden" name="edit_id" value="<?= $editData['id'] ?>">

                <button name="update" class="bg-[#01a9ac] hover:bg-[#07e4e8] text-white text-xs px-4 py-2  transition-all duration-300">Save</button>

            <?php else: ?>
                <button name="save" class="bg-[#01a9ac] hover:bg-[#07e4e8] text-white text-xs px-4 py-2  transition-all duration-300">Save</button>
            <?php endif; ?>
        </form>
    </div>

    <!-- Category Table -->
    <div class="lg:col-span-8 md:col-span-8 sm:col-span-12 col-span-12 bg-white rounded shadow p-4 border-t-4 border-teal-500">
        <h2 class="font-medium text-gray-700 mb-4">Category List</h2>
        <table class="w-full text-sm text-left border">
            <thead >
            <tr >
                <th class="p-2 font-medium border">Sr</th>
                <th class="p-2 font-medium border">Category Name</th>
                <th class="p-2 font-medium border text-center">Action</th>
            </tr>
            </thead>
            <tbody class="border ">
            <?php $i = 1; while ($row = $categories->fetch_assoc()): ?>
                <tr class="border-b ">
                    ...
                    <td class="p-2 border capitalize"><?= htmlspecialchars($row['name']) ?></td>
                    <td class="p-2 border text-center ">
                        <a href="?edit=<?= $row['id'] ?>" class="bg-[#01a9ac] hover:bg-[#07e4e8] text-white text-xs px-4 py-2 transition-all duration-300">Edit</a>
                        <a href="?delete=<?= $row['id'] ?>" onclick="return confirm('Are you sure?')" class="bg-[#fe5d70] text-white px-3 py-2 text-xs hover:bg-[#fe5d70] transition-all duration-300">Delete</a>
                    </td>
                </tr>
            <?php endwhile; ?>
            </tbody>
