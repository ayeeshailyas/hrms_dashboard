...
$month = isset($_GET['month']) ? $_GET['month'] : 'July';
...
<div id="mainContent" class=" p-8 absolute top-1 ">
    ...
    if (isset($_GET['status'])) {
        if ($_GET['status'] === 'added') $toastMsg = '🎉 Holiday saved successfully!';
        elseif ($_GET['status'] === 'updated') $toastMsg = '✅ Holiday updated successfully!';
        elseif ($_GET['status'] === 'deleted') $toastMsg = '🗑️ Holiday deleted successfully!';
    }
    ?>
    ...
    <div class="grid grid-cols-12 gap-3">
        ...
        <!-- Months Section -->
        <div id="monthList" class="lg:col-span-3 md:col-span-3 sm:col-span-12 col-span-12">
            ...
            foreach ($months as $m) {
                $isActive = ($m === $month);
                $activeClass = $isActive ? 'bg-[#816F95]' : 'bg-[rgba(60,109,132,0.87)] hover:bg-[#816F95]';

                echo "<a href='#' data-month='$m' class='relative group block month-link'>";


                echo "<div class='flex items-stretch h-[42px] border-b border-white cursor-pointer transition-all duration-300 $activeClass'>";

                // Icon box
                echo "<div class='w-10 flex justify-center items-center text-white' style='background-color: #816F95;'>
            <i class='fa fa-calendar'></i>
        </div>";

                // Month name
                echo "<div class='flex-1 px-4 py-2 text-sm font-medium text-white flex items-center'>
            $m
        </div>";
                ...
                if ($isActive) {
                    echo "<div class='absolute top-2 right-0 w-0 h-0 border-l-[10px] border-l-transparent border-b-[10px] border-b-white'></div>";
                    echo "<div class='absolute top-[17px] right-0 w-0 h-0 border-l-[10px] border-l-transparent border-t-[10px] border-t-white'></div>";
                }
        ...
        <div id="holidayTableSection" class=" lg:col-span-9 md:col-span-9 sm:col-span-12 col-span-12 bg-white  rounded-lg shadow p-6 self-start">
    ...
    document.querySelectorAll('.month-link').forEach(link => {
        link.addEventListener('click', function (e) {
            ...
            fetch('holiday_data.php', {
                ...
                .then(res => res.text())
                .then(html => {
                    document.getElementById('holidayTableSection').innerHTML = html;

                    // Update styling of active month
                    document.querySelectorAll('#monthList .month-link').forEach(link => {
                        const div = link.querySelector('div.flex');
                        if (div) {
                            div.classList.remove('bg-[#816F95]');
                            div.classList.add('bg-[rgba(60,109,132,0.87)]', 'hover:bg-[#816F95]');
                        }

                        // Remove triangle arrows
                        const arrows = link.querySelectorAll('.absolute');
                        arrows.forEach(el => el.remove());
                    });

                    // Set the clicked one to active style
                    const activeDiv = this.querySelector('div.flex');
                    if (activeDiv) {
                        activeDiv.classList.remove('bg-[rgba(60,109,132,0.87)]', 'hover:bg-[#816F95]');
                        activeDiv.classList.add('bg-[#816F95]');
                    }

                    // Re-add triangle arrows
                    const triangle1 = document.createElement('div');
                    triangle1.className = 'absolute top-2 right-0 w-0 h-0 border-l-[10px] border-l-transparent border-b-[10px] border-b-white';
                    const triangle2 = document.createElement('div');
                    triangle2.className = 'absolute top-[17px] right-0 w-0 h-0 border-l-[10px] border-l-transparent border-t-[10px] border-t-white';
                    this.appendChild(triangle1);
                    this.appendChild(triangle2);
                });
        });
    });
</script>
