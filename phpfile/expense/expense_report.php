...
<div id="mainContent" class="p-8 absolute top-1">
    ...
    if (isset($_GET['status']) && $_GET['status'] === 'added') {
        $toastMsg = '✅ Expense saved successfully!';
    }
    ?>

    <?php if ($toastMsg): ?>
        <div id="toast" class="fixed top-5 right-5 bg-black text-white px-6 py-3 rounded shadow-lg opacity-0 transform translate-x-full transition-all duration-700 z-50">
            <?= $toastMsg ?>
        </div>
        <script>
    ...
    <h1 class="font-semibold text-[24px]">Expense Report</h1>
    <span>
        <p class="float-right text-[#919aa3] text-[15px]">
            <a href="#"><i class="fa fa-home text-[#4a6076]"></i></a> /
            <a href="#">Expense</a>
        </p>
    </span>
    ...
    <div class="grid grid-cols-12 gap-3">
        ...
        <div class="lg:col-span-3 md:col-span-3 sm:col-span-12 col-span-12">
            ...
            foreach ($months as $m): ?>
                <a href="javascript:void(0)" onclick="loadMonth('<?= $m ?>')" id="month-<?= $m ?>" class="relative group block">
                    <div class="month-tab flex items-stretch h-[42px] border-b border-white cursor-pointer transition-all duration-300"
                         style="background-color: rgba(60,109,132,0.87);"
                         onmouseover="this.style.backgroundColor='#816F95';"
                         onmouseout="resetMonthTab(this, '<?= $m ?>');">
                        <!-- Icon -->
                        <div class="w-10 flex justify-center items-center text-white" style="background-color: #816F95;">
                            <i class="fa fa-calendar"></i>
                        </div>
                        <!-- Text -->
                        <div class="flex-1 px-4 py-2 text-sm font-medium text-white flex items-center"><?= $m ?></div>
                    </div>

                    <!-- Triangle indicators -->
                    <div id="triangle-<?= $m ?>" class="hidden absolute top-2 right-0 w-0 h-0 border-l-[10px] border-l-transparent border-b-[10px] border-b-white"></div>
                    <div id="triangle2-<?= $m ?>" class="hidden absolute top-[17px] right-0 w-0 h-0 border-l-[10px] border-l-transparent border-t-[10px] border-t-white"></div>
                </a>
            <?php endforeach; ?>
        ...
        <div class="lg:col-span-9 md:col-span-9 sm:col-span-12 col-span-12 bg-white rounded-lg shadow p-6 self-start">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-md font-medium text-gray-800">
                    <i class="fa fa-calendar-days mr-2"></i>
                    <span id="selectedMonth"><?= date('F') ?></span>
                </h1>
                <div>
                    <a href="#" onclick="generatePDF()" class="bg-[#01a9ac] text-white text-[14px] px-3 py-2.5  hover:bg-[#07e4e8] transition-all"> <i class="fa fa-file-pdf-o"></i></a>
                    <a href="#" onclick="window.print()" class="bg-[#01a9ac] text-white text-[14px] px-3 py-2.5  hover:bg-[#07e4e8] transition-all ml-2"> <i class="fa fa-print"></i></a>
                </div>
            </div>

            <div class="overflow-x-auto" id="expenseData">
                <!-- Data will be loaded here via AJAX -->
                <p class="text-gray-500 text-center py-8">Select a month to view expenses.</p>
            </div>
        </div>
    </div>
</div>
...
<script>
        ...
        let activeMonth = '<?= date('F') ?>';

        function resetMonthTab(tabElement, month) {
        if (month !== activeMonth) {
        tabElement.style.backgroundColor = 'rgba(60,109,132,0.87)';
    }
    }
        ...
        // 1. Update selected month text
        document.getElementById('selectedMonth').innerText = month;

        // 2. Reset all month tabs
        document.querySelectorAll('.month-tab').forEach(el => {
        el.style.backgroundColor = 'rgba(60,109,132,0.87)';
    });

        // 3. Highlight active month tab
        const activeTab = document.querySelector(`#month-${month} .month-tab`);
        if (activeTab) {
        activeTab.style.backgroundColor = '#816F95';
    }

        // 4. Reset all triangle indicators
        document.querySelectorAll("[id^='triangle']").forEach(tri => tri.classList.add("hidden"));

        // 5. Show triangle for active month
        document.getElementById(`triangle-${month}`)?.classList.remove("hidden");
        document.getElementById(`triangle2-${month}`)?.classList.remove("hidden");

        // 6. Load expense table for that month
        $.ajax({
        url: 'fetch_expense.php',
        method: 'GET',
        data: { month: month },
        success: function (res) {
        $('#expenseData').html(res);
    }
    });
    }

        // On page load, load current month and highlight it
...
<script>
    async function generatePDF() {
        ...
        // Create a plain text version of table for now (or use html2canvas if needed)
        let y = 10;
        doc.setFontSize(12);
        doc.text("Expense Report - " + document.getElementById('selectedMonth').innerText, 10, y);
        y += 10;
