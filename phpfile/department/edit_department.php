...
<?php
$conn = mysqli_connect("localhost", "root", "", "hrms_dashboard");
if (!$conn) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $designations = array_filter(array_map('trim', $_POST['designations']));

    if ($new_department && !empty($designations)) {
        // Delete old department data
        $stmt = mysqli_prepare($conn, "DELETE FROM department_designation WHERE department_name = ?");
        mysqli_stmt_bind_param($stmt, "s", $old_department);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);

        // Insert updated department data
        $stmt = mysqli_prepare($conn, "INSERT INTO department_designation (department_name, designation_name) VALUES (?, ?)");
        foreach ($designations as $desig) {
            mysqli_stmt_bind_param($stmt, "ss", $new_department, $desig);
            mysqli_stmt_execute($stmt);
        }
        mysqli_stmt_close($stmt);

        $_SESSION['toast'] = "Department updated successfully!";
...
// Handle edit display
$edit_department = isset($_GET['edit']) ? $_GET['edit'] : '';
$edit_department = trim($edit_department);
$designations = [];

if ($edit_department) {
    $stmt = mysqli_prepare($conn, "SELECT designation_name FROM department_designation WHERE department_name = ?");
    mysqli_stmt_bind_param($stmt, "s", $edit_department);
    mysqli_stmt_execute($stmt);
...
<div id="mainContent" class=" p-8 absolute top-1 ">
    <h1 class="font-medium text-[22px]">Add Department</h1>
    <span>
<!--        <p class="float-left text-[14px] text-[#919aa3] pt-2">Notice Board</p>-->
    ...
    <div class=" bg-white p-6  w-full  rounded shadow ">
        <!--  Toast Notification -->
        <!-- Success Notification -->
        <div id="successToast"
             class="fixed top-6 right-6 bg-black bg-opacity-80 text-white text-sm px-5 py-4 rounded shadow-lg z-50
            transform translate-x-full opacity-0 transition-all duration-500 ease-in-out flex items-center gap-2">
        ...
        <form id="editForm" method="POST" class="space-y-4 ">
            <!-- Hidden Department -->
            <input type="hidden" name="old_department" value="<?= htmlspecialchars($edit_department) ?>">

            <!-- Department Row -->
            <div class="lg:flex md:flex items-center lg:space-x-4 md:space-x-4">
                <label class="w-48 text-[15px] text-gray-700 sm:pb-2 lg:pb-0 md:pb-0 pb-2">Add Department <span class="text-red-500">*</span></label>
                <input type="text" name="department" value="<?= htmlspecialchars($edit_department) ?>" placeholder="Enter Your Department"
                       class="w-[50%] py-1 px-3 rounded-none border border-gray-300 focus:border-[#01a9ac]" />
            </div>

            <!-- First Designation -->
            <?php $first = isset($designations[0]) ? $designations[0] : ''; ?>
            <div class="lg:flex md:flex items-center lg:space-x-4 md:space-x-4">
                <label class="w-48 text-[15px] text-gray-700 sm:pb-2 lg:pb-0 md:pb-0 pb-2">Add Designation <span class="text-red-500">*</span></label>
                <input type="text" name="designations[]" value="<?= htmlspecialchars($first) ?>" placeholder="Enter Your Designations"
                       class="w-[50%] py-1 px-3 rounded-none border border-gray-300 focus:border-[#01a9ac]" />
                <button type="button" onclick="addDesignation()" class="text-sm text-[#404E67] hover:text-[#01a9ac] font-bold whitespace-nowrap">
            ...
            <div id="designationsContainer" class="space-y-4">
                <?php for ($i = 1; $i < count($designations); $i++): ?>
                    <div class="lg:flex md:flex items-center lg:space-x-4 md:space-x-4">
                        <label class="w-48 text-[15px] text-gray-700">Add Designation *</label>
                        <input type="text" name="designations[]" value="<?= htmlspecialchars($designations[$i]) ?>" placeholder="Enter Your Designations"
                               class="w-[50%] py-1 px-3 rounded-none border border-gray-300 focus:border-[#01a9ac]" />
                        <button type="button" onclick="this.parentElement.remove()" class="text-sm text-[#404E67] hover:text-[#01a9ac] font-bold whitespace-nowrap ml-2">
            ...
            <div class="text-start pt-2">
                <button type="submit" class="bg-[#01a9ac] hover:bg-[#07e4e8] text-white text-sm px-4 py-2.5 transition-all duration-300">Save</button>
            </div>
        </form>
...
<script>
    function addDesignation() {
        ...
        div.className = 'lg:flex md:flex items-center lg:space-x-4 md:space-x-4';
        div.innerHTML = `
            <label class="w-48 text-[15px] text-gray-700 ">Add Designation *</label>
            <input type="text" name="designations[]" placeholder="Enter Your Designations"
                   class="w-[50%] py-1 px-3 rounded-none border border-gray-300 focus:border-[#01a9ac]" />
            <button type="button" onclick="this.parentElement.remove()"
                    class="text-sm text-[#404E67] hover:text-[#01a9ac] font-bold whitespace-nowrap ml-2">
                <i class="fa fa-times"></i> Remove
            </button>
        `;
