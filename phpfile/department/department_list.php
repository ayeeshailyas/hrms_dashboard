...
// Fetch grouped data
$result = mysqli_query($conn, "SELECT * FROM department_designation ORDER BY department_name, id ASC");
$departments = [];
$seen = [];
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (!isset($seen[$dept])) {
        $seen[$dept] = count($seen); // assign order
    }
    $departments[$dept][] = $row;
}

// Sort departments in the order they were seen
uksort($departments, function ($a, $b) use ($seen) {
    if ($seen[$a] == $seen[$b]) {
        return 0;
    }
    return ($seen[$a] < $seen[$b]) ? -1 : 1;
});
...
<?php if (isset($_SESSION['toast'])): ?>
    <div id="successToast"
         class="fixed top-6 right-6 bg-black bg-opacity-80 text-white text-sm px-6 py-4 rounded shadow-lg z-50
...
<div id="mainContent" class=" p-8 absolute top-1 ">
    <h1 class="font-medium text-[22px]">Departments</h1>
    <span>
        <p class="float-left text-[14px] text-[#919aa3] pt-2">All Departments</p>
    ...
    <div class=" bg-white w-full rounded ">
        <!-- Single white box for all departments -->
        <div class="  rounded  border-t-4 border-[#01a9ac]">
            <?php foreach ($departments as $dept => $rows): ?>
                <!-- Department header -->
                <div class="flex items-center justify-between p-4 ">
                    <h2 class="text-xl font-semibold text-gray-700"><?= htmlspecialchars($dept) ?></h2>
                    <div class="space-x-2">
                        <!-- Parent container must allow overflow -->
                        <div class="relative overflow-visible z-0 flex items-center space-x-2">

                            <!-- EDIT BUTTON -->
                            <div class="relative group inline-block">
                                <a href="edit_department.php?edit=<?= urlencode($dept) ?>"
                                   class="bg-[#01a9ac] hover:bg-[#07e4e8] text-white text-[13px] transition-all ease-in duration-300 rounded-[2px] px-3 py-2 flex items-center">
                                    <i class="fa fa-pencil-square-o mr-1"></i> Edit
                                </a>

                                <!-- Tooltip -->
                                <div class="absolute -top-10 left-1/2 transform -translate-x-1/2  opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    border-l-transparent border-r-transparent border-t-gray-700"></div>
                            ...
                            <!-- DELETE BUTTON -->
                            <div class="relative group inline-block">
                                <a href="?delete_dept=<?= urlencode($dept) ?>"
                                ...
                                <!-- Tooltip -->
                                <div class="absolute -top-10 left-1/2 transform -translate-x-1/2 z-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    <div class="relative bg-gray-700 text-white text-xs rounded px-2 py-1">
                                        Delete
                                        <div class="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0
                    border-l-4 border-r-4 border-t-4
                ...
                <!-- Designations Table -->
                <table class="w-full text-left  border-[#e9ecef]">
                    <thead>
                    <thead>
                    <tr>
                        <th class="px-4 py-4 text-sm font-medium text-gray-600 border-b-2 border-[#ccc] border-t  border-r">
                            Sr.
                        </th>
                        <th class="px-4 py-4 text-sm font-medium text-gray-600 border-b-2 border-[#ccc] border-t border-r">
                            Designation
                    ...
                    <?php foreach ($rows as $index => $row): ?>
                        <tr class="hover:bg-gray-100 ">
                            <td class="px-4 py-4 text-sm text-gray-700  border border-[#e9ecef]"><?= $index + 1 ?></td>
                            <td class="px-4 py-4 text-sm text-gray-700 border border-[#e9ecef]"><?= htmlspecialchars($row['designation_name']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
