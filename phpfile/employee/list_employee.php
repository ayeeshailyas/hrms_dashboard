...
<div id="toast" class="fixed top-5 right-[-400px] bg-black text-white px-6 py-3 rounded shadow transition-all duration-500 z-50">
...
<script>
    function showToast(message) {
        ...
        toast.classList.remove("right-[-400px]");
        toast.classList.add("right-5");

        setTimeout(() => {
            toast.classList.remove("right-5");
            toast.classList.add("right-[-400px]");
        }, 3000);
    }
...
<div id="mainContent" class=" p-8 absolute top-1 ">
    <h1 class="font-semibold text-[24px]">Employees</h1>
    <span>
<!--    <p class="text-[#919aa3] text-[14px] pt-2 float-left">Holiday List</p>-->
    ...
    <div class="w-full mx-auto bg-white shadow p-6 rounded-md border-t-4 border-teal-500">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-md font-medium text-gray-800">Employee List</h1>
            <div>
        ...
        <table class="table-bordered border-[#ececec] w-full text-sm" cellpadding="10">
            <thead>
            <tr>
                <th  class="font-medium">EMP ID</th>
                <th  class="font-medium">Employee</th>
                <th class="font-medium">Dept. > Designation</th>
                <th class="font-medium">Mobile</th>
                <th class="text-center font-medium">Status</th>
                <th class="text-center font-medium">View</th>
                <th class="text-center font-medium">Action</th>
            </tr>
            </thead>
            <tbody>
            <?php while($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td class="text-center">
                            <span class="badge " style="background: <?= $row['status'] === 'Active' ? 'linear-gradient(to right, #0ac282, #0df3a3)' : '#6c757d' ?>; color: white; padding: 8px 12px;">
                         <?= $row['status'] ?></span>
                    </td>
                    <td><a href="view_employee.php?id=<?= $row['id'] ?>" class="btn bg-[#2DCEE3] px-[14px] py-1.5 rounded-1 hover:bg-[#2fdef6] btn-sm text-white"><i class="fa fa-list-alt"></i></a></td>
                    <td class="text-center">
                         <a href="delete_employee.php?id=<?= $row['id'] ?>" onclick="return confirm('Are you sure you want to delete this employee?')" class="bg-[#fe5d70] rounded-1 text-white px-3 py-2.5 text-xs hover:bg-[#fe5d70] transition-all duration-300"><i class="fa fa-trash-o"></i> Delete</a>
                    </td>
