...
// Fetch designations grouped by department
...
// Fetch awards list
$awardResult = $conn->query("SELECT a.*, e.f_name, e.l_name, e.emp_id FROM employee_awards a JOIN employee e ON a.employee_id = e.id");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<style>
    ...
    /*!* 🔹 Keep caret white for both *!*/
    /*.bootstrap-select .dropdown-toggle::after,*/
    /*#employeeDropdown::after {*/
    /*    color: white !important;*/
    /*}*/

    /*!* 🔹 Dropdown menu itself *!*/
    /*.bootstrap-select .dropdown-menu,*/
    /*#employeeDropdown + .dropdown-menu {*/
    ...
    /*!* ✅ Selected item inside menu *!*/
    /*.bootstrap-select .dropdown-menu li.selected a,*/
    /*#employeeDropdown + .dropdown-menu li.selected a {*/
    ...
    /*#employeeDropdown + .dropdown-menu li a:hover {*/
    /*    background-color: #01a9ac !important;*/
    /*    color: white !important;*/
    /*}*/
    ...
    /* 🔹 Base style for both select elements */
    #designationDropdown, #employeeDropdown {
        background-color: #01a9ac !important;
        color: white !important;
        border: none !important;
        border-radius: 0px !important;
    ...
    /* 🔹 Focus state */
    #designationDropdown:focus, #employeeDropdown:focus {
        outline: none !important;
        box-shadow: none !important;
        border-color: #01a9ac !important;
    }
    ...
    #designationDropdown option, #employeeDropdown option {
        background-color: white;
        color: black;
        padding: 8px 12px;
    }
   ...
   /* #designationDropdown option:hover, #employeeDropdown option:hover,*/
   /*{*/
   ...
   /* }*/
    ...
    #designationDropdown option:checked, #employeeDropdown option:checked{
        background-color: #e2e3e5;
        color: black;
    }

        /* 🔹 Disabled department headers */
    #designationDropdown option[disabled] {
        background-color: white;
        color: #495057;
        font-weight: bold;
    }
...
<div id="mainContent" class=" p-8 absolute top-1 ">
    ...
    <div class="max-w-7xl mx-auto ">
        ...
        <div class="bg-white rounded-lg   border-t-4 border-[#01a9ac] shadow-[0_1px_20px_0_rgba(69,90,100,0.08)] p-6">
            ...
            <form method="POST" action="save_award.php" class="max-w-3xl    p-6 rounded-lg  space-y-6">
                <?php
                $conn = new mysqli("localhost", "root", "", "hrms_dashboard");
                if ($conn->connect_error) {
                    die("Connection failed: " . $conn->connect_error);
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
                    ...
                    <select name="designation" id="designationDropdown" class="col-span-3 form-control ">
                        <option selected>Select Designations.....</option>
                        <?php
                        foreach ($grouped as $department => $designations) {
                            echo "<option disabled class='bg-gray-100 font-bold text-black'>$department</option>";
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
                    ...
                    <select name="employee_id" id="employeeDropdown" class="col-span-3 form-control">
                        <option selected>Select Employee...</option>
                    </select>
                </div>
                <!-- Award Name -->
                <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
                    <label class="text-gray-700 font-medium col-span-1">Award Name / Title <span
                                class="text-red-500">*</span></label>
                    <input type="text" name="award_name" required
                           class="col-span-3 w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4 relative">
                    ...
                    <div class="col-span-3 flex items-center w-full relative">
                        <input id="dobInput" name="award_month" placeholder="Enter Month" readonly
                               class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500">
                    ...
                    <div id="calendar"
                         class="hidden absolute top-full mt-2 left-[45%] w-[200px] h-[215px] bg-white rounded-xl shadow-xl z-50 pt-2 text-center flex flex-col">
...
<!--                 Submit Button -->
                <div class="text-center">
                    <button type="submit"
                            class="bg-[#01a9ac] hover:bg-[#0dcaf0] text-white  transition-all ease-in duration-300 rounded-[2px] px-6 py-2 flex items-center ml-48 text-[17px] ">
                        Save
                    </button>
                </div>
            </form>
        ...
        <div class="mt-6 bg-white p-6 rounded-md border border-gray-200 shadow-sm">
            <h2 class="text-gray-600 font-semibold mb-4">Employees Who Received Awards</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full border">
                    <thead class=" text-gray-600  text-sm">
                    <tr>
                        <th class="border px-4 py-3">Emp ID</th>
                        <th class="border px-4 py-2">Employee Name</th>
                        <th class="border px-4 py-2">Award Name</th>
                        <th class="border px-4 py-2">Gift</th>
                        <th class="border px-4 py-2">Award Amount</th>
                        <th class="border px-4 py-2">Month</th>
                        <th class="border px-4 py-2">Action</th>
                    </tr>
                    </thead>
                    <tbody class="text-sm text-gray-700">
                    <?php while ($row = $awardResult->fetch_assoc()): ?>
                        <tr>
                            <td class="border px-4 py-2"><?= $row['employee_id'] ?></td>
                            <td class="border px-4 py-2"><?= $row['f_name'] . " " . $row['l_name'] ?></td>
                            <td class="border px-4 py-2"><?= $row['award_name'] ?></td>
                            <td class="border px-4 py-2"><?= $row['gift_item'] ?></td>
                            <td class="border px-4 py-2"><?= $row['award_amount'] ?></td>
                            <td class="border px-4 py-2"><?= date('M y', strtotime($row['award_month'] . '-01')) ?></td>
                            <td class="border px-4 py-2 flex gap-2">


                                <!-- EDIT BUTTON -->
                                <div class="relative group inline-block">
                                    <a href="edit_award.php?id=<?= $row['id'] ?>"
                                       class="bg-[#01a9ac] hover:bg-[#0dcaf0] text-white text-[13px] transition-all ease-in duration-300 rounded-[2px] px-3 py-2 flex items-center">
                                        <i class="fa fa-pencil-square-o mr-1"></i> Edit
                                    </a>

                                    <!-- Tooltip -->
                                    <div class="absolute -top-10 left-1/2 transform -translate-x-1/2 z-999 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                        <div class="relative bg-gray-700 text-white text-xs rounded px-2 py-1">
                                            Edit
                                            <div class="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0
                    border-l-4 border-r-4 border-t-4
                    border-l-transparent border-r-transparent border-t-gray-700"></div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- DELETE BUTTON -->
                                <div class="relative group inline-block">
                                    <a href="delete_award.php?id=<?= $row['id'] ?>"
                                       onclick="return confirm('You are about to delete this Department. All Designations under this Department will be deleted. Are you sure?');"
                                       class="bg-[#fe5d70] hover:bg-[#fe5d70]/80 text-white text-[13px] transition-all ease-in duration-300 rounded-[2px] px-3 py-2 flex items-center">
                                        <i class="fa fa-trash-o mr-1"></i> Delete
                                    </a>

                                    <!-- Tooltip -->
                                    <div class="absolute -top-10 left-1/2 transform -translate-x-1/2 z-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                        <div class="relative bg-gray-700 text-white text-xs rounded px-2 py-1">
                                            Delete
                                            <div class="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0
                    border-l-4 border-r-4 border-t-4
...
<script>
    document.getElementById('designationDropdown').addEventListener('change', function () {
        ...
        // Show loading state
        employeeDropdown.innerHTML = '<option selected>Loading employees...</option>';

        fetch('get_employees.php?designation=' + encodeURIComponent(designation))
            .then(response => {
                if (!response.ok) throw new Error('Network response was not ok');
                return response.json();
            })
            .then(data => {
                employeeDropdown.innerHTML = '<option selected>Select Employee...</option>';

                if (data && data.length > 0) {
                    data.forEach(emp => {
                        let option = document.createElement('option');
                        option.value = emp.id;
                        option.textContent = `${emp.f_name} ${emp.l_name}`;
                        employeeDropdown.appendChild(option);
                    });
                } else {
                    employeeDropdown.innerHTML = '<option selected>No employees found</option>';
                }
            })
            .catch(error => {
                console.error('Error:', error);
                employeeDropdown.innerHTML = '<option selected>Error loading employees</option>';
            });
    });
</script>
