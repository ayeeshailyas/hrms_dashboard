...
$desig_result = $conn->query($sql);
$grouped = [];
if ($desig_result->num_rows > 0) {
    while ($d = $desig_result->fetch_assoc()) {
        $grouped[$d['department_name']][] = $d['designation_name'];
    }
}
?>
<!DOCTYPE html>
...
<style>
    ...
    .hover-zone1:hover .hover-icon1 {
        opacity: 1;
        transform: translateY(-50%) scale(1.2);
    }
    .zoom-text1 { transition: transform 0.2s ease; display: inline-block; }
    .hover-zone1:hover .zoom-text1 { transform: scale(1.37); }
    .slide-text1 { display: inline-block; position: relative; }

    .slide-from-left1 {
        animation: slideFromLeft1 0.25s ease-out forwards;
    }
    .slide-from-right1 {
        animation: slideFromRight1 0.25s ease-out forwards;
    }
    ...
    .zoom-in1 { animation: zoomIn1 0.3s ease forwards; }
    .zoom-out1 { animation: zoomOut1 0.3s ease forwards; }
    ...
    .no-scrollbar1::-webkit-scrollbar { display: none; }
    .no-scrollbar1 { -ms-overflow-style: none; scrollbar-width: none; }

    #gridView1.grid-3rows1 { flex: 1; overflow-y: auto; }
    #calendar1 {
        border: 1px solid #1abc9c !important;
        box-shadow: 0 0 10px 0 rgba(0, 136, 204, 0.45) !important;
        border-radius: 8px !important;
    }
    .b11 { border-left: 1px solid #1abc9c !important; border-top: 1px solid #1abc9c !important; }

    .scrollbar-thin1::-webkit-scrollbar { width: 4px; }
    ...
    .notification1.show1 { right: 20px; }
        ...
        <div class="grid grid-cols-12 gap-4">
            ...
            <div class="lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                ...
                <div class="w-full mx-auto bg-white shadow p-6 rounded-md border-t-4 border-teal-500">
                    <h3 class="text-md font-medium mb-4">Personal Details</h3>
                    <div class="space-y-4">
                        <div>
                            <label class="w-48 text-[15px] text-gray-700">First Name <span class="text-red-500">*</span></label>
                            <input type="text" name="f_name" value="<?= htmlspecialchars($row['f_name']) ?>" required class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>

                        <div>
                            <label class="w-48 text-[15px] text-gray-700">Last Name <span class="text-red-500">*</span></label>
                            <input type="text" name="l_name" value="<?= htmlspecialchars($row['l_name']) ?>" required class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>



                        <div class="relative">
                            <label class="text-gray-700 w-40">Date of Birth   <span class="text-red-500">*</span></label>
                            <div class="w-full flex items-center relative">
                                <input id="dobInput1" name="dob" placeholder="YYYY-MM"  value="<?= $row['dob'] ?>" readonly
                            ...
                            <div id="calendar1" class="hidden absolute top-full mt-2 left-[35%] w-[200px] h-[215px] bg-white rounded-xl shadow-xl z-50 pt-2 text-center flex flex-col">
                                <div class="absolute -top-2 left-1/2 transform b11 -translate-x-1/2 w-4 h-4 bg-white rotate-45"></div>

                                <div id="mainView1">
                                    <div class="border-b hover-zone1 py-1" onclick="toggleGrid1('month')">
                                        <div class="hover-icon1 cursor-pointer left-2 text-teal-500 text-xl font-bold" onclick="event.stopPropagation(); changeMonth1(-1)">&#10094;</div>
                                        <div id="monthScroll1" class="text-[18px] font-bold text-black zoom-text1 cursor-pointer slide-text1">Jul</div>
                                        <div class="hover-icon1 cursor-pointer right-2 text-teal-500 text-xl font-bold" onclick="event.stopPropagation(); changeMonth1(1)">&#10095;</div>
                                    </div>
                                    <div class="border-b hover-zone1 py-1" onclick="toggleGrid1('date')">
                                        <div class="hover-icon1 cursor-pointer left-2 text-teal-500 text-xl font-bold" onclick="event.stopPropagation(); changeDate1(-1)">&#10094;</div>
                                        <div id="dateScroll1" class="text-4xl font-bold text-teal-500 zoom-text1 cursor-pointer slide-text1">11</div>
                                        <div id="dayName1" class="text-xs font-bold mt-0.5">Friday</div>
                                        <div class="hover-icon1 cursor-pointer right-2 text-teal-500 text-xl font-bold" onclick="event.stopPropagation(); changeDate1(1)">&#10095;</div>
                                    </div>
                                    <div class="hover-zone1 py-1" onclick="toggleGrid1('year')">
                                        <div class="hover-icon1 cursor-pointer left-2 text-teal-500 text-xl font-bold" onclick="event.stopPropagation(); changeYear1(-1)">&#10094;</div>
                                        <div id="yearScroll1" class="text-base text-gray-700 mt-3 zoom-text1 font-bold cursor-pointer slide-text1">2025</div>
                                        <div class="hover-icon1 cursor-pointer right-2 text-teal-500 text-xl font-bold" onclick="event.stopPropagation(); changeYear1(1)">&#10095;</div>
                                    </div>
                                </div>

                                <div id="gridView1" class="hidden px-3 pb-3 max-h-[215px] no-scrollbar1"></div>
                                <div id="actionBtn1" class="bg-teal-500 w-full mt-auto rounded-b-lg py-2 text-white text-2xl font-bold cursor-pointer">✓</div>
                            </div>
                        </div>
                        ...
                        <div>
                            <label class="w-40 text-[15px] text-gray-700">Gender</label>
                            <select name="gender" class="selectpicker form-control rounded-none w-full" data-live-search="true"
                        ...
                        <div>
                            <label class="w-40 text-[15px] text-gray-700">Marital Status</label>
                            <select name="marital_status" class="selectpicker form-control rounded-none w-full" data-live-search="true"
                                    title="Select Country">
                                <option value="" selected>Select Status...</option>
                                <option value="Married" <?= $row['marital_status'] === 'Married' ? 'selected' : '' ?>>Married</option>
                                <option value="Unmarried" <?= $row['marital_status'] === 'Unmarried' ? 'selected' : '' ?>>Unmarried</option>
                        ...
                        <div>
                            <label class="w-48 text-[15px] text-gray-700">Father's Name <span class="text-red-500">*</span></label>
                            <input type="text" name="father_name" value="<?= htmlspecialchars($row['father_name']) ?>" required class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>

                        <div>
                            <label class="w-40 text-[15px] text-gray-700">Nationality</label>
                            <select name="nationality" value="<?= htmlspecialchars($row['nationality']) ?>" class="selectpicker form-control rounded-none w-full text-sm "  data-live-search="true" title="Select Country">
                                <option value="" selected>Select Country...</option>
                                <option value="Afghanistan" <?= $row['nationality'] === 'Afghanistan' ? 'selected' : '' ?>>Afghanistan</option>
                                <option value="Albania" <?= $row['nationality'] === 'Albania' ? 'selected' : '' ?>>Albania</option>
                                <option value="Algeria" <?= $row['nationality'] === 'Algeria' ? 'selected' : '' ?>>Algeria</option>
                                <option value="Andorra" <?= $row['nationality'] === 'Andorra' ? 'selected' : '' ?>>Andorra</option>
                                <option value="Angola" <?= $row['nationality'] === 'Angola' ? 'selected' : '' ?>>Angola</option>
                                <option value="Argentina" <?= $row['nationality'] === 'Argentina' ? 'selected' : '' ?>>Argentina</option>
                                <option value="Armenia" <?= $row['nationality'] === 'Armenia' ? 'selected' : '' ?>>Armenia</option>
                                <option value="Australia" <?= $row['nationality'] === 'Australia' ? 'selected' : '' ?>>Australia</option>
                                <option value="Austria" <?= $row['nationality'] === 'Austria' ? 'selected' : '' ?>>Austria</option>
                                <option value="Azerbaijan" <?= $row['nationality'] === 'Azerbaijan' ? 'selected' : '' ?>>Azerbaijan</option>
                                <option value="Bahamas" <?= $row['nationality'] === 'Bahamas' ? 'selected' : '' ?>>Bahamas</option>
                                <option value="Bahrain" <?= $row['nationality'] === 'Bahrain' ? 'selected' : '' ?>>Bahrain</option>
                                <option value="Bangladesh" <?= $row['nationality'] === 'Bangladesh' ? 'selected' : '' ?>>Bangladesh</option>
                                <option value="Barbados" <?= $row['nationality'] === 'Barbados' ? 'selected' : '' ?>>Barbados</option>
                                <option value="Belarus" <?= $row['nationality'] === 'Belarus' ? 'selected' : '' ?>>Belarus</option>
                                <option value="Belgium" <?= $row['nationality'] === 'Belgium' ? 'selected' : '' ?>>Belgium</option>
                                <option value="Belize" <?= $row['nationality'] === 'Belize' ? 'selected' : '' ?>>Belize</option>
                                <option value="Benin" <?= $row['nationality'] === 'Benin' ? 'selected' : '' ?>>Benin</option>
                                <option value="Bhutan" <?= $row['nationality'] === 'Bhutan' ? 'selected' : '' ?>>Bhutan</option>
                                <option value="Bolivia" <?= $row['nationality'] === 'Bolivia' ? 'selected' : '' ?>>Bolivia</option>
                                <option value="Bosnia" <?= $row['nationality'] === 'Bosnia' ? 'selected' : '' ?>>Bosnia</option>
                                <option value="Pakistan" <?= $row['nationality'] === 'Pakistan' ? 'selected' : '' ?>>Pakistan</option>
                                <option value="Palau" <?= $row['nationality'] === 'Palau' ? 'selected' : '' ?>>Palau</option>
                                <option value="Palestine" <?= $row['nationality'] === 'Palestine' ? 'selected' : '' ?>>Palestine</option>
                                <option value="Panama" <?= $row['nationality'] === 'Panama' ? 'selected' : '' ?>>Panama</option>
                                <!-- Add more countries as needed -->
                            </select>
                        </div>

                        <div>
                            <label class="w-48 text-[15px] text-gray-700">Passport Number <span class="text-red-500">*</span></label>
                            <input type="text" name="pass_no" value="<?= htmlspecialchars($row['passport_no']) ?>" required class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>

                        <div>
                            <label class="w-40 text-[15px] text-gray-700">Photo <span class="text-danger">*</span></label>
                            <input type="file" name="photo" class=" w-full" >
                            <?php if (!empty($row['photo'])): ?>
                                <p>Current: <?= $row['photo'] ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <!-- Bank Information -->
                <div class="w-full mx-auto bg-white shadow p-6 rounded-md border-t-4 border-teal-500">
                    <h3 class="text-md font-medium mb-4">Bank Information</h3>
                    <div class="space-y-4">
                        <div>
                            <label class="w-48 text-[14px] text-gray-700">Bank Name</label>
                            <input type="text" name="bank_name" value="<?= htmlspecialchars($row['bank_name']) ?>" class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>

                        <div>
                            <label class="w-48 text-[14px] text-gray-700">Branch Name</label>
                            <input type="text" name="branch_name" value="<?= htmlspecialchars($row['branch_name']) ?>" class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>

                        <div>
                            <label class="w-48 text-[14px] text-gray-700">Account Name</label>
                            <input type="text" name="account_name" value="<?= htmlspecialchars($row['account_name']) ?>" class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>

                        <div>
                            <label class="w-48 text-[14px] text-gray-700">Account Number</label>
                            <input type="text" name="account_number"  value="<?= htmlspecialchars($row['account_number']) ?>" class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-6 md:col-span-6 sm:col-span-12 col-span-12">
                ...
                <!-- Contact Details -->
                <div class="w-full mx-auto bg-white shadow p-6 rounded-md border-t-4 border-teal-500">
                    <h3 class="text-md font-medium mb-4">Contact Details</h3>
                    <div class="space-y-4">
                        <div>
                            <label class="w-48 text-[15px] text-gray-700">Permanent Address <span class="text-red-500">*</span></label>
                            <textarea name="address" required class="w-full form-control rounded-none border border-gray-300"><?= htmlspecialchars($row['address']) ?></textarea>
                        </div>

                        <div>
                            <label class="w-48 text-[15px] text-gray-700">City <span class="text-red-500">*</span></label>
                            <input type="text" name="city" value="<?= htmlspecialchars($row['city']) ?>" required class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>


                        <div>
                            <label class="w-40 text-[15px] text-gray-700">Country</label>
                            <select name="contact_nationality" class="selectpicker form-control rounded-none w-full text-sm "  data-live-search="true" title="Select Country">
                                <option value="" selected>Select Country...</option>
                                <option value="Afghanistan" <?= $row['contact_nationality'] === 'Afghanistan' ? 'selected' : '' ?>>Afghanistan</option>
                                <option value="Albania" <?= $row['contact_nationality'] === 'Albania' ? 'selected' : '' ?>>Albania</option>
                                <option value="Algeria" <?= $row['contact_nationality'] === 'Algeria' ? 'selected' : '' ?>>Algeria</option>
                                <option value="Andorra" <?= $row['contact_nationality'] === 'Andorra' ? 'selected' : '' ?>>Andorra</option>
                                <option value="Angola" <?= $row['contact_nationality'] === 'Angola' ? 'selected' : '' ?>>Angola</option>
                                <option value="Argentina" <?= $row['contact_nationality'] === 'Argentina' ? 'selected' : '' ?>>Argentina</option>
                                <option value="Armenia" <?= $row['contact_nationality'] === 'Armenia' ? 'selected' : '' ?>>Armenia</option>
                                <option value="Australia" <?= $row['contact_nationality'] === 'Australia' ? 'selected' : '' ?>>Australia</option>
                                <option value="Austria" <?= $row['contact_nationality'] === 'Austria' ? 'selected' : '' ?>>Austria</option>
                                <option value="Azerbaijan" <?= $row['contact_nationality'] === 'Azerbaijan' ? 'selected' : '' ?>>Azerbaijan</option>
                                <option value="Bahamas" <?= $row['contact_nationality'] === 'Bahamas' ? 'selected' : '' ?>>Bahamas</option>
                                <option value="Bahrain" <?= $row['contact_nationality'] === 'Bahrain' ? 'selected' : '' ?>>Bahrain</option>
                                <option value="Bangladesh" <?= $row['contact_nationality'] === 'Bangladesh' ? 'selected' : '' ?>>Bangladesh</option>
                                <option value="Barbados" <?= $row['contact_nationality'] === 'Barbados' ? 'selected' : '' ?>>Barbados</option>
                                <option value="Belarus" <?= $row['contact_nationality'] === 'Belarus' ? 'selected' : '' ?>>Belarus</option>
                                <option value="Belgium" <?= $row['contact_nationality'] === 'Belgium' ? 'selected' : '' ?>>Belgium</option>
                                <option value="Belize" <?= $row['contact_nationality'] === 'Belize' ? 'selected' : '' ?>>Belize</option>
                                <option value="Benin" <?= $row['contact_nationality'] === 'Benin' ? 'selected' : '' ?>>Benin</option>
                                <option value="Bhutan" <?= $row['contact_nationality'] === 'Bhutan' ? 'selected' : '' ?>>Bhutan</option>
                                <option value="Bolivia" <?= $row['contact_nationality'] === 'Bolivia' ? 'selected' : '' ?>>Bolivia</option>
                                <option value="Bosnia" <?= $row['contact_nationality'] === 'Bosnia' ? 'selected' : '' ?>>Bosnia</option>
                                <option value="Pakistan" <?= $row['contact_nationality'] === 'Pakistan' ? 'selected' : '' ?>>Pakistan</option>
                                <option value="Palau" <?= $row['contact_nationality'] === 'Palau' ? 'selected' : '' ?>>Palau</option>
                                <option value="Palestine" <?= $row['contact_nationality'] === 'Palestine' ? 'selected' : '' ?>>Palestine</option>
                                <option value="Panama" <?= $row['contact_nationality'] === 'Panama' ? 'selected' : '' ?>>Panama</option>
                                <!-- Add more countries as needed -->
                            </select>
                        </div>

                        <div>
                            <label class="w-48 text-[15px] text-gray-700">Mobile <span class="text-red-500">*</span></label>
                            <input type="text" name="mobile" value="<?= htmlspecialchars($row['mobile']) ?>" required class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>

                        <div>
                            <label class="w-48 text-[15px] text-gray-700">Phone <span class="text-red-500">*</span></label>
                            <input type="text" name="phone" value="<?= htmlspecialchars($row['phone']) ?>" required class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>

                        <div>
                            <label class="w-48 text-[15px] text-gray-700">Email <span class="text-red-500">*</span></label>
                            <input type="email" name="email" value="<?= htmlspecialchars($row['email']) ?>" required class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>
                    </div>
                </div>
                ...
                <!-- Documents -->
                <div class="w-full mx-auto bg-white shadow p-6 rounded-md border-t-4 border-teal-500">
                    <h3 class="text-md font-medium mb-4">Employee Documents</h3>
                    <div class="space-y-4">

                        <div class="flex items-center space-x-2" id="resume-section">
                            <label class="w-48 text-[15px] text-gray-700 pb-2">Resume</label>

                            <!-- Hidden File Input -->
                            <input type="file" name="resume" id="resume-upload" class="hidden" onchange="handleFileChange(event, 'resume')" />

                            <!-- Label Acting As Button -->
                            <label id="resume-label" for="resume-upload"
                                   class="cursor-pointer text-sm bg-[#cecece] hover:bg-[#38f1f6] !text-black hover:!text-white py-2 px-3 transition duration-300">
                                Select File
                            </label>


                            <!-- File Info Display -->
                            <div id="resume-file-info" class="hidden flex items-center space-x-2">
                                <!--                                <span id="resume-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"></span>-->
                                <?php if (!empty($row['resume'])): ?>
                                    <span id="resume-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"> <?= $row['resume'] ?></span>
                                <?php endif; ?>
                                <button type="button" onclick="removeFile('resume')" class="text-red-500 hover:text-red-700 text-lg font-bold">×</button>
                            </div>
                        </div>


                        <!-- OFFER LETTER -->
                        <div class="flex items-center space-x-2" id="offer-section">
                            <label class="w-48 text-[15px] text-gray-700 pb-2">Offer Letter</label>
                            <input type="file" name="offer_letter" id="offer-upload" class="hidden" onchange="handleFileChange(event, 'offer')" />
                            <label id="offer-label" for="offer-upload"
                                   class="cursor-pointer text-sm bg-[#cecece] hover:bg-[#38f1f6] !text-black hover:!text-white py-2 px-3 transition duration-300">
                                Select File
                            </label>
                            <div id="offer-file-info" class="hidden flex items-center space-x-2">
                                <!--                                <span id="offer-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"></span>-->
                                <?php if (!empty($row['offer_Letter'])): ?>
                                    <span id="offer-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"> <?= $row['offer_Letter'] ?></span>
                                <?php endif; ?>
                                <button type="button" onclick="removeFile('offer')" class="text-red-500 hover:text-red-700 text-lg font-bold">×</button>
                            </div>
                        </div>

                        <!-- JOINING LETTER -->
                        <div class="flex items-center space-x-2" id="joining-section">
                            <label class="w-48 text-[15px] text-gray-700 pb-2">Joining Letter</label>
                            <input type="file" name="joining_letter" id="joining-upload" class="hidden" onchange="handleFileChange(event, 'joining')" />
                            <label id="joining-label" for="joining-upload"
                                   class="cursor-pointer text-sm bg-[#cecece] hover:bg-[#38f1f6] !text-black hover:!text-white py-2 px-3 transition duration-300">
                                Select File
                            </label>
                            <div id="joining-file-info" class="hidden flex items-center space-x-2">
                                <!--                                <span id="joining-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"></span>-->
                                <?php if (!empty($row['joining_letter'])): ?>
                                    <span id="joining-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"><?= $row['joining_letter'] ?></span>
                                <?php endif; ?>
                                <button type="button" onclick="removeFile('joining')" class="text-red-500 hover:text-red-700 text-lg font-bold">×</button>
                            </div>
                        </div>

                        <!-- CONTRACT PAPER -->
                        <div class="flex items-center space-x-2" id="contract-section">
                            <label class="w-48 text-[15px] text-gray-700 pb-2">Contract Paper</label>
                            <input type="file" name="contract_paper" id="contract-upload" class="hidden" onchange="handleFileChange(event, 'contract')" />
                            <label id="contract-label" for="contract-upload"
                                   class="cursor-pointer text-sm bg-[#cecece] hover:bg-[#38f1f6] !text-black hover:!text-white py-2 px-3 transition duration-300">
                                Select File
                            </label>
                            <div id="contract-file-info" class="hidden flex items-center space-x-2">
                                <!--                                <span id="contract-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"></span>-->
                                <?php if (!empty($row['contract_paper'])): ?>
                                    <span id="contract-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"><?= $row['contract_paper'] ?></span>
                                <?php endif; ?>
                                <button type="button" onclick="removeFile('contract')" class="text-red-500 hover:text-red-700 text-lg font-bold">×</button>
                            </div>
                        </div>

                        <!-- ID PROOF -->
                        <div class="flex items-center space-x-2" id="idproof-section">
                            <label class="w-48 text-[15px] text-gray-700 pb-2">ID Proof</label>
                            <input type="file" name="id_proof" id="idproof-upload" class="hidden" onchange="handleFileChange(event, 'idproof')" />
                            <label id="idproof-label" for="idproof-upload"
                                   class="cursor-pointer text-sm bg-[#cecece] hover:bg-[#38f1f6] !text-black hover:!text-white py-2 px-3 transition duration-300">
                                Select File
                            </label>
                            <div id="idproof-file-info" class="hidden flex items-center space-x-2">
                                <!--                                <span id="idproof-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"></span>-->
                                <?php if (!empty($row['id_proof'])): ?>
                                    <span id="idproof-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"><?= $row['id_proof'] ?></span>
                                <?php endif; ?>
                                <button type="button" onclick="removeFile('idproof')" class="text-red-500 hover:text-red-700 text-lg font-bold">×</button>
                            </div>
                        </div>

                        <!-- OTHER DOCUMENT -->
                        <div class="flex items-center space-x-2" id="otherdoc-section">
                            <label class="w-48 text-[15px] text-gray-700 pb-2">Other Document</label>
                            <input type="file" name="other_document" id="otherdoc-upload" class="hidden" onchange="handleFileChange(event, 'otherdoc')" />
                            <label id="otherdoc-label" for="otherdoc-upload"
                                   class="cursor-pointer text-sm bg-[#cecece] hover:bg-[#38f1f6] !text-black hover:!text-white py-2 px-3 transition duration-300">
                                Select File
                            </label>
                            <div id="otherdoc-file-info" class="hidden flex items-center space-x-2">
                                <!--                                <span id="otherdoc-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"></span>-->
                                <?php if (!empty($row['other_document'])): ?>
                                    <span id="otherdoc-file-name" class="text-sm text-gray-700 truncate max-w-[200px]"><?= $row['other_document'] ?></span>
                                <?php endif; ?>
                                <button type="button" onclick="removeFile('otherdoc')" class="text-red-500 hover:text-red-700 text-lg font-bold">×</button>
                            </div>
                        </div>
                ...
                <!-- Official Status -->
                <div class="w-full mx-auto bg-white shadow p-6 rounded-md border-t-4 border-teal-500">
                    <h3 class="text-md font-medium mb-4">Official Status</h3>
                    <div class="space-y-4">

                        <div class=" relative">
                            <label class="text-gray-700  ">Joining Date <span class="text-red-500">*</span></label>
                            <div class="w-full flex items-center   relative">
                                <input id="dobInput" name="join_date" placeholder="YYYY-MM" value="<?= $row['join_date'] ?>" readonly
                                       class="w-full px-3 py-2 border  focus:outline-none focus:ring-2 focus:ring-teal-500">
                                <div class="absolute right-2 text-teal-600 ">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                                         stroke="currentColor">
                        ...
                        <div>
                            <label class="text-[15px] text-gray-700">Employee ID <span class="text-red-500">*</span></label>
                            <input type="text" name="emp_id" required class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>
                        ...
                        <div>
                            <label class="text-[15px] text-gray-700">Employee ID <span class="text-red-500">*</span></label>
                            <input type="text" name="emp_id" value="<?= htmlspecialchars($row['emp_id']) ?>" required class="form-control w-full rounded-none py-1 border border-gray-300" />
                        </div>

                        <label class="w-40 text-[15px] mt-2 text-gray-700 sm:pb-2  lg:pb-0 md:pb-0">Designation<span
                                    class="text-red-500">*</span></label>

                        <select name="designation_combined" class="selectpicker form-control rounded-[2px] w-full" data-live-search="true">
                            ...
                            <?php foreach ($grouped as $dept => $designations): ?>
                                ...
                                <?php foreach ($designations as $designation):
                                    ...
                                    $selected = ($row['department'] == $dept && $row['designation'] == $designation) ? 'selected' : '';
                                    ?>
                                    <option value="<?= $value ?>" <?= $selected ?>>&nbsp;&nbsp;&nbsp;<?= $designation ?></option>
                                <?php endforeach; ?>
                            <?php endforeach; ?>
                        </select>
...
<!--                        <div>-->
<!--                            <label class="text-[15px] text-gray-700">Joining Date <span class="text-red-500">*</span></label>-->
<!--                            <input type="date" name="join_date" value="--><?php //= $row['join_date'] ?><!--" required class="form-control w-full rounded-none py-1 border border-gray-300" />-->
<!--                        </div>-->
                    </div>
                </div>
            </div>
        ...
        <div class="text-start pt-4">
            <button type="submit" class="bg-[#cecece] w-full hover:bg-[#07e4e8] hover:!text-white text-black px-6 py-2 transition duration-300">
                Update
            </button>
        </div>
    </form>
...
<script>
    ...
    actionBtn.onclick = () => {
        if (gridView.classList.contains("hidden")) {
            ...
            // Modified this line to show month number instead of name
            dobInput.value = `${tempDate.getFullYear()}-${String(tempDate.getMonth() + 1).padStart(2, "0")}-${String(tempDate.getDate()).padStart(2, "0")}`;
...
<script>
    const dobInput1 = document.getElementById("dobInput1");
    const calendar1 = document.getElementById("calendar1");
    const monthScroll1 = document.getElementById("monthScroll1");
    const dateScroll1 = document.getElementById("dateScroll1");
    const yearScroll1 = document.getElementById("yearScroll1");
    ...
    function updateCalendar1() {
        monthScroll1.innerText = months1[tempDate1.getMonth()];
        dateScroll1.innerText = String(tempDate1.getDate()).padStart(2, "0");
        yearScroll1.innerText = tempDate1.getFullYear();

        const weekday1 = tempDate1.getDay();
        const weekdayStr1 = tempDate1.toLocaleDateString("en-US", { weekday: "long" });
        dayName1.innerText = weekdayStr1;

        const isWeekend1 = weekday1 === 0 || weekday1 === 6;
        dayName1.className = `text-xs font-bold mt-0.5 ${isWeekend1 ? "text-teal-500" : "text-gray-900"}`;
        dateScroll1.className = `text-4xl font-bold zoom-text1 cursor-pointer slide-text1 ${isWeekend1 ? "text-teal-500" : "text-gray-900"}`;
    }

    function animateChange1(el, newText, direction) {
        if (isAnimating1) return;
        isAnimating1 = true;
        const animClass = direction === "right" ? "slide-from-right1" : "slide-from-left1";
        el.classList.remove("slide-from-left1", "slide-from-right1");
        void el.offsetWidth;
        el.classList.add(animClass);
        el.innerText = newText;
        setTimeout(() => { isAnimating1 = false; }, 250);
    }

    function changeMonth1(offset) {
        const direction = offset > 0 ? "right" : "left";
        tempDate1.setMonth(tempDate1.getMonth() + offset);
        animateChange1(monthScroll1, months1[tempDate1.getMonth()], direction);
        updateCalendar1();
    }

    function changeDate1(offset) {
        const direction = offset > 0 ? "right" : "left";
        tempDate1.setDate(tempDate1.getDate() + offset);
        animateChange1(dateScroll1, String(tempDate1.getDate()).padStart(2, "0"), direction);
        updateCalendar1();
    }

    function changeYear1(offset) {
        const direction = offset > 0 ? "right" : "left";
        tempDate1.setFullYear(tempDate1.getFullYear() + offset);
        animateChange1(yearScroll1, tempDate1.getFullYear(), direction);
        updateCalendar1();
    }

    dobInput1.onclick = () => {
        calendar1.classList.toggle("hidden");
        tempDate1 = new Date(currentDate1);
        updateCalendar1();
    };

    actionBtn1.onclick = () => {
        if (gridView1.classList.contains("hidden")) {
            currentDate1 = new Date(tempDate1);
            // Modified this line to show month number instead of name
            dobInput1.value = `${tempDate1.getFullYear()}-${String(tempDate1.getMonth() + 1).padStart(2, "0")}-${String(tempDate1.getDate()).padStart(2, "0")}`;
            calendar1.classList.add("hidden");
        } else {
            closeGrid1();
        }
    };

    function toggleGrid1(type) {
        if (!gridView1.classList.contains("hidden")) return;
        mainView1.classList.add("hidden");
        gridView1.innerHTML = "";
        gridView1.classList.remove("hidden", "zoom-out1");
        gridView1.classList.add("zoom-in1", "grid-3rows1");
        ...
        if (type === "month") {
            html += "<div class='grid grid-cols-2 gap-1'>";
            months1.forEach((m, i) => {
                html += `<div onclick='selectGrid1("month", ${i})' class='pt-1 border-b rounded cursor-pointer font-semibold text-sm text-black flex flex-col items-center'>${m}<span class="text-xs text-black">${String(i + 1).padStart(2, "0")}</span></div>`;
            });
            html += "</div>";
        } else if (type === "date") {
            const daysInMonth1 = new Date(tempDate1.getFullYear(), tempDate1.getMonth() + 1, 0).getDate();
            html += "<div class='grid grid-cols-2 gap-1'>";
            for (let i = 1; i <= daysInMonth1; i++) {
                const dateObj = new Date(tempDate1.getFullYear(), tempDate1.getMonth(), i);
                const weekdayName1 = dateObj.toLocaleDateString("en-US", { weekday: "short" }).toUpperCase();
                const isWeekend1 = dateObj.getDay() === 0 || dateObj.getDay() === 6;
                const dayTextColor = isWeekend1 ? "text-teal-500" : "text-gray-900";

                html += `<div onclick='selectGrid1("date", ${i})' class='border-b rounded cursor-pointer font-bold flex flex-col items-center text-sm ${dayTextColor}'>${String(i).padStart(2, "0")}<span class="text-[10px] text-black">${weekdayName1}</span></div>`;
            }
            html += "</div>";
        } else if (type === "year") {
            html += "<div class='grid grid-cols-2 gap-1'>";
            for (let i = 1970; i <= 2020; i += 10) {
                html += `<div onclick='loadDecade1(${i})' class='border-b rounded p-2 cursor-pointer font-semibold text-base text-gray-900 text-center'>${i}</div>`;
            }
            html += "</div>";
        }
    ...
    function loadDecade1(startYear) {
        let html = "<div class='grid grid-cols-2 gap-1'>";
        for (let i = startYear; i < startYear + 10; i++) {
            html += `<div onclick='selectGrid1("year", ${i})' class='border-b p-2 rounded cursor-pointer font-semibold text-base text-gray-900 text-center'>${i}</div>`;
        }
        html += "</div>";
        gridView1.innerHTML = html;
    }

    function selectGrid1(type, value) {
        if (type === "month") tempDate1.setMonth(value);
        if (type === "date") tempDate1.setDate(value);
        if (type === "year") tempDate1.setFullYear(value);
        updateCalendar1();
        closeGrid1();
    }

    function closeGrid1() {
        gridView1.classList.remove("zoom-in1", "grid-3rows1");
        gridView1.classList.add("zoom-out1");
        setTimeout(() => {
            gridView1.classList.add("hidden");
            mainView1.classList.remove("hidden");
            actionBtn1.innerText = "✓";
        }, 250);
    }
</script>
